        ...
        <section class="relative h-screen flex items-center justify-center parallax-bg">
            ...
            <div class="relative z-10 text-center px-4" x-data="{ show: false }" x-init="setTimeout(() => show = true, 500)">
                <div :class="show ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'" class="transition-all duration-1000 transform">
                    <p class="text-olive font-sans tracking-widest uppercase text-xl md:text-3xl mb-4 font-bold drop-shadow-sm">
                        ¡Hola, <span class="font-black text-2xl md:text-4xl">{{ $guest->name }}</span>!
                    </p>
                        ...
                        <form class="space-y-6" x-data="{ showConfirmModal: false }" @submit.prevent="showConfirmModal = true">
                            ...
                            <div>
                                ...
                                <div class="flex gap-4">
                                    <label class="flex-1 flex items-center justify-center gap-2 cursor-pointer p-3 rounded-xl border-2 transition-all duration-200 transform active:scale-95 hover:scale-[1.02]"
                                           @click="rsvp_attending = true"
                                           :class="rsvp_attending ? 'border-olive bg-olive/10 text-olive font-semibold shadow-inner' : 'border-gray-200 hover:border-gray-300'">
                                        <input type="radio" wire:model="rsvp_attending" value="1" class="hidden">
                                        <span>✓ Sí, ahí estaré</span>
                                    </label>
                                    <label class="flex-1 flex items-center justify-center gap-2 cursor-pointer p-3 rounded-xl border-2 transition-all duration-200 transform active:scale-95 hover:scale-[1.02]"
                                           @click="rsvp_attending = false"
                                           :class="!rsvp_attending ? 'border-red-400 bg-red-50 text-red-600 font-semibold shadow-inner' : 'border-gray-200 hover:border-gray-300'">
                            ...
                            <div x-show="showConfirmModal" x-cloak class="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm" style="z-index: 99999;"
                                ...
                                <div @click.away="showConfirmModal = false" class="bg-white p-8 rounded-3xl shadow-2xl max-w-sm w-full mx-4 text-center transform transition-all"
                                     x-transition:enter="transition ease-out duration-300"
                                     x-transition:enter-start="opacity-0 translate-y-8 scale-95"
                                     x-transition:enter-end="opacity-100 translate-y-0 scale-100">
                                    <div class="w-16 h-16 bg-olive/10 rounded-full flex items-center justify-center mx-auto mb-4 text-olive">
                                    ...
                                    <h3 class="font-serif text-2xl text-olive mb-2">¿Estás seguro?</h3>
