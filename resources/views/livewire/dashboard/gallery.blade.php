    ...
    <div class="bg-ivory font-sans text-gray-800 overflow-x-hidden min-h-screen pt-24" x-data="{ imgModal: false, imgModalSrc: '' }">
        ...
        <section class="py-20 bg-wood/10 min-h-screen relative z-10">
            <div class="container mx-auto px-4">
                <h2 x-data="{ visible: false }" x-intersect.once="visible = true" :class="visible ? 'visible' : ''" class="font-script text-5xl md:text-7xl text-olive mb-4 text-center fade-in-up">Nuestros Momentos</h2>
                <p x-data="{ visible: false }" x-intersect.once="visible = true" :class="visible ? 'visible' : ''" class="text-center font-serif text-lg italic text-gray-600 mb-10 fade-in-up">Sube tus fotos de la boda y sé parte de nuestros recuerdos.</p>
                
                <!-- Uploader Section -->
                <div x-data="{ visible: false }" x-intersect.once="visible = true" :class="visible ? 'visible' : ''" class="max-w-xl mx-auto mb-16 bg-white p-6 rounded-2xl shadow-lg border border-beige fade-in-up">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4" wire:poll.10s>
                    ...
                    @forelse ($this->photos as $index => $photoUrl)
                        <div wire:key="photo-{{ md5($photoUrl) }}" x-data="{ visible: false }" x-intersect.once="visible = true" :class="visible ? 'visible' : ''" class="aspect-w-1 aspect-h-1 overflow-hidden rounded-lg shadow-sm cursor-pointer group fade-in-up" 
